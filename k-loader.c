...
static int program_load_segment(proc* p, const elf_program* ph,
    ...
    for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
        uintptr_t page = PAGEADDRESS(next_available_phys_page());
        if (assign_physical_page(page, p->p_pid) < 0
    ...
    memset((uint8_t*) end_file, 0, end_mem - end_file);
     ...
     // Loop through program headers to find read-only segments
    if ((ph->p_flags & ELF_PFLAG_WRITE) == 0) {
        // Iterate over the segment’s pages and map each as read-only
        for (uintptr_t addr = va; addr < end_mem; addr += PAGESIZE) {
            vamapping m = virtual_memory_lookup(p->p_pagetable, addr);
            // Attempt to map each page with read-only permissions
            if (virtual_memory_map(p->p_pagetable, addr, m.pa, PAGESIZE, PTE_P | PTE_U) < 0) {
