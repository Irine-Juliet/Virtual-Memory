...
int virtual_memory_map(x86_64_pagetable* pagetable, uintptr_t va,
    ...
    for (; sz != 0; va += PAGESIZE, pa += PAGESIZE, sz -= PAGESIZE) {
        ...
        if ((perm & PTE_P) && l1pagetable) { // if page is marked present
            ...
            l1pagetable->entry[index] = (uint64_t) pa | (uint64_t) perm ; 
        } else if (l1pagetable) { // if page is NOT marked present
            // TODO
            // map to address 0 with `perm`
            int index = L1PAGEINDEX(va);
            l1pagetable->entry[index] = 0 | perm; // l1pagetable->entry[L1PAGEINDEX(va)] = (uint64_t) 0 | (uint64_t) perm;
        } else if (perm & PTE_P) {
...
static x86_64_pagetable* lookup_l1pagetable(x86_64_pagetable* pagetable,
    ...
    for (int i = 0; i <= 2; ++i) {
        ...
        x86_64_pageentry_t pe = pt->entry[index]; // replace this (was set to 0)  (x86_64_pageentry_t) pt->entry[PAGEINDEX(va, i)];
        ...
        pt = (x86_64_pagetable*) (PTE_ADDR(pe)); // replace this (was set to 0)
    }
    return pt;
}
